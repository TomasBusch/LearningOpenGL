...
Batch::Batch()
	:is_Empty(true), MAX_VERTICES(pow(2, 15)), MAX_INDICES(pow(2, 15))
{

}

Batch::Batch(unsigned int max_vertices, unsigned int max_indices)
	:is_Empty(true), MAX_VERTICES(max_vertices), MAX_INDICES(max_indices)
{
...
void Batch::generateBatch(VertexBuffer& vb, IndexBuffer& ib) {
	...
	for (auto& model : m_ModelsDrawQueue) {
		for (auto& mesh : model->getMeshes()) {
			...
			ib.SetData(index_offset * sizeof(unsigned int), sizeof(unsigned int) * indices.size(), indices.data());
